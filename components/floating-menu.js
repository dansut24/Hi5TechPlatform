...
export default function FloatingMenu({ navItems, activeNav, onSwitchPage, user, onGoModules, onLogout, menuOpen, setMenuOpen, onOpenSearch, themeMode, setThemeMode, customTheme, setCustomTheme, theme }) {
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2">
        ...
        {menuOpen ? (
          <div className="fixed inset-0 z-40 bg-black/55 backdrop-blur-[2px]" onClick={() => setMenuOpen(false)}>
            <motion.div
              ...
              className={cn("absolute bottom-24 left-6 z-40 w-[420px] max-w-[calc(100vw-24px)] rounded-[32px] border p-3 shadow-2xl shadow-black/50", theme.panel)}
