...
function ActionButton({ children, theme, secondary = false }) {
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center rounded-2xl px-4 py-2.5 text-sm transition",
        secondary
  ...
  const filtered = useMemo(
    () =>
      incidents.filter((incident) =>
        `${incident.id} ${incident.title} ${incident.priority} ${incident.assignee}`.toLowerCase().includes(query.toLowerCase())
      ),
    [query]
  )
        ...
        <div className="grid gap-3 md:grid-cols-[1fr,200px,auto]">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search incidents, teams, assignees..."
        ...
        <ShellCard theme={theme} className="p-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <InputShell theme={theme} value={form.shortDescription} onChange={(e) => setForm({ ...form, shortDescription: e.target.value })} placeholder="VPN access failing for remote users" />
            </div>
            <div>
              <div className={cn("mb-2 text-sm", theme.muted)}>Requester</div>
              <InputShell theme={theme} value={form.requester} onChange={(e) => setForm({ ...form, requester: e.target.value })} placeholder="Alex Morgan" />
            </div>
            <div>
              <div className={cn("mb-2 text-sm", theme.muted)}>Priority</div>
              <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className={cn("h-11 w-full rounded-2xl border px-4 text-sm outline-none", theme.input)}>
                <option>Low</option>
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={form.details} onChange={(e) => setForm({ ...form, details: e.target.value })} className={cn("min-h-[160px] w-full rounded-2xl border px-4 py-3 text-sm outline-none", theme.input)} />
            </div>
          </div>
          <div className="mt-5 flex gap-3">
            <ActionButton theme={theme}>Submit incident</ActionButton>
        ...
        <ShellCard theme={theme} className="p-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select value={form.requestType} onChange={(e) => setForm({ ...form, requestType: e.target.value })} className={cn("h-11 w-full rounded-2xl border px-4 text-sm outline-none", theme.input)}>
            ...
            <div>
              ...
              <InputShell theme={theme} value={form.requestedFor} onChange={(e) => setForm({ ...form, requestedFor: e.target.value })} placeholder="Jamie Carter" />
            </div>
            <div className="md:col-span-2">
              <div className={cn("mb-2 text-sm", theme.muted)}>Notes</div>
              <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className={cn("min-h-[120px] w-full rounded-2xl border px-4 py-3 text-sm outline-none", theme.input)} />
            </div>
          </div>
          <div className="mt-5 flex gap-3">
            <ActionButton theme={theme}>Submit request</ActionButton>
            <ActionButton theme={theme} secondary>Cancel</ActionButton>
          </div>
        </ShellCard>
        <ShellCard theme={theme} className="p-5">
    ...
    <div className="space-y-6">
      ...
      <ShellCard theme={theme} className="p-5">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {items.map((item) => (
            <div key={item} className={cn("rounded-2xl border p-4 text-sm", theme.subCard, theme.line)}>
              {item}
            </div>
          ))}
        </div>
  ...
  if (moduleId === "control") {
    return (
      <GenericWorkspace
        ...
        items={["Device overview", "Remote tools", "Patch compliance", "Alert queue"]}
        icon={Monitor}
      />
    )
  }
  ...
  if (moduleId === "selfservice") {
    return (
      <GenericWorkspace
        theme={theme}
        title="SelfService"
        subtitle="End-user support, requests, and knowledge."
        items={["Raise incident", "Request software", "Request hardware", "Search knowledge"]}
        icon={UserCircle2}
      />
    )
  }
