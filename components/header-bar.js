      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className={cn("absolute right-0 top-12 z-40 min-w-[280px] rounded-3xl border p-3 shadow-2xl shadow-black/40", theme.panel)}
          >
              ...
              {navItems.slice(0, 6).map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn("flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left transition", selected ? theme.selected : theme.hover)}
                  >
...
function NotificationBell({ theme }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className={cn("relative flex h-9 w-9 items-center justify-center rounded-xl border transition", theme.card, theme.hover)}
      >
