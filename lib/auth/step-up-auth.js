...
export async function getOrCreateUserSecuritySettings(userId) {
  if (!userId) return null

  const supabase = await createServerSupabaseClient()
  const existing = await getUserSecuritySettings(userId)
  ...
  if (error) {
    console.error("[step-up] failed to create user security settings", error)
    return null
  }
...
export function buildOtpAuthUri({ email, tenantName, secret }) {
  return authenticator.keyuri(email, tenantName || "Hi5Tech", secret)
}
...
export async function savePendingTotpEnrollment({ userId, secret }) {
  const cookieStore = await cookies()
  cookieStore.set(
    `hi5_pending_totp_${userId}`,
    JSON.stringify({
      secret,
      issuedAt: Date.now(),
    }),
...
export async function getPendingTotpEnrollment(userId) {
  const cookieStore = await cookies()
  const raw = cookieStore.get(`hi5_pending_totp_${userId}`)?.value || null
  if (!raw) return null
...
export async function clearPendingTotpEnrollment(userId) {
  const cookieStore = await cookies()
  cookieStore.set(`hi5_pending_totp_${userId}`, "", {
    httpOnly: true,
    secure: true,
    sameSite: "lax",
    path: "/",
